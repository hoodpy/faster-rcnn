...
class Vgg16():
	...
	def _anchor_component(self):
		with tf.compat.v1.variable_scope("ANCHOR_default"):
			height = tf.cast(tf.math.ceil(self._im_info[0, 0] / np.float32(self._feat_stride[0])), tf.int32)
			width = tf.cast(tf.math.ceil(self._im_info[0, 1] / np.float32(self._feat_stride[0])), tf.int32)
	...
	def _proposal_target_layer(self, rois, roi_scores, name):
		with tf.compat.v1.variable_scope(name):
			...
			self._proposal_targets["labels"] = tf.cast(labels, tf.int32, name="to_int32")
	...
	def _crop_pool_layer(self, bottom, rois, name):
		with tf.compat.v1.variable_scope(name):
			...
			height = (tf.cast(bottom_shape[1], tf.float32) - 1.0) * np.float32(self._feat_stride[0])
			width = (tf.cast(bottom_shape[2], tf.float32) - 1.0) * np.float32(self._feat_stride[0])
	...
	def build_network(self, sess, is_training=True):
		with tf.compat.v1.variable_scope("vgg_16", "vgg_16"):
	...
	def _add_losses(self, sigma_rpn=3.0):
		with tf.compat.v1.variable_scope("loss_" + self._tag):
			...
			self._losses["total_loss"] = cross_entropy + loss_box + rpn_cross_entropy + rpn_loss_box
	...
	def fix_variables(self, sess, pretrained_model):
		...
		with tf.compat.v1.variable_scope("Fix_VGG16"):
			with tf.device("/cpu:0"):
				...
				restorer_fc = tf.compat.v1.train.Saver({"vgg_16/fc6/weights": fc6_conv, "vgg_16/fc7/weights": fc7_conv,
					"vgg_16/conv1/conv1_1/weights": conv1_rgb})
				restorer_fc.restore(sess, pretrained_model)
				sess.run(tf.compat.v1.assign(self._variables_to_fix["vgg_16/fc6/weights:0"], tf.reshape(fc6_conv,
					self._variables_to_fix["vgg_16/fc6/weights:0"].get_shape())))
				sess.run(tf.compat.v1.assign(self._variables_to_fix["vgg_16/fc7/weights:0"], tf.reshape(fc7_conv,
					self._variables_to_fix["vgg_16/fc7/weights:0"].get_shape())))
				sess.run(tf.compat.v1.assign(self._variables_to_fix["vgg_16/conv1/conv1_1/weights:0"], tf.reverse(conv1_rgb, [2])))

	def create_architecture(self, sess, mode, num_classes, tag=None, anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2)):
		self._image = tf.compat.v1.placeholder(tf.float32, shape=[self._batch_size, None, None, 3])
		self._im_info = tf.compat.v1.placeholder(tf.float32, shape=[self._batch_size, 3])
		...
		if testing:
			...
			tf.compat.v1.summary.scalar("rpn_loss_box", self._losses["rpn_loss_box"])
			tf.compat.v1.summary.scalar("rpn_cross_entropy", self._losses["rpn_cross_entropy"])
			tf.compat.v1.summary.scalar("loss_box", self._losses["loss_box"])
			tf.compat.v1.summary.scalar("cross_entropy", self._losses["cross_entropy"])
		return layers_to_output

	def train_step(self, sess, blobs, train_op, merged, global_step, run_options=None, run_metadata=None):
		feed_dict = {self._image: blobs["data"], self._im_info: blobs["im_info"], self._gt_boxes: blobs["gt_boxes"]}
		if run_options == None or run_metadata == None:
			summary, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss, _, steps = sess.run([merged, self._losses["rpn_cross_entropy"], 
			self._losses["rpn_loss_box"], self._losses["cross_entropy"], self._losses["loss_box"], self._losses["total_loss"], 
			train_op, global_step], feed_dict=feed_dict)
		else:
			summary, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss, _, steps = sess.run([merged, self._losses["rpn_cross_entropy"], 
				self._losses["rpn_loss_box"], self._losses["cross_entropy"], self._losses["loss_box"], self._losses["total_loss"], 
				train_op, global_step], feed_dict=feed_dict, options=run_options, run_metadata=run_metadata)
		return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss, summary, steps
...
class Train():
	def __init__(self):
		self._xml_path = "D:/program/faster_rcnn/VOC2007/Annotations"
		self._file_path = "D:/program/faster_rcnn/VOC2007/JPEGImages"
		self._xml_list = os.listdir(self._xml_path)
		self._num_samples = len(self._xml_list)
		self._classes = ['__background__', 'aeroplane', 'bicycle', 'bird', 'boat', 'bottle', 'bus', 'car', 'cat', 'chair', 'cow', 
		'diningtable', 'dog', 'horse', 'motorbike', 'person', 'pottedplant', 'sheep', 'sofa', 'train', 'tvmonitor']
		self._ckpt_file = "D:/program/faster_rcnn/vgg16.ckpt"
		self._output_dir = "D:/program/faster_rcnn/model/"
		self._log_dir = "D:/program/faster_rcnn/log"
	...
	def parse_xml(self, xml_file):
		...
		image_name = os.path.join(self._file_path, name)
	...
	def train(self):
		...
		with sess.graph.as_default():
			...
			layers = self.net.create_architecture(sess, "TRAIN", 21, tag="default")
			loss = layers["total_loss"]
			lr = tf.compat.v1.Variable(self._learning_rate, trainable=False)
			global_step = tf.compat.v1.Variable(0, trainable=False)
			tf.compat.v1.summary.scalar("learning_rate", lr)
			tf.compat.v1.summary.scalar("global_step", global_step)
			optimizer = tf.compat.v1.train.MomentumOptimizer(lr, self._momentum)
			gvs = optimizer.compute_gradients(loss)
			final_gvs = []
			with tf.compat.v1.variable_scope("Gradient_Mult"):
				for grad, var in gvs:
					scale = 1.0
					if "/biases:" in var.name:
						scale *= 2.0
					if not np.allclose(scale, 1.0):
						grad = tf.math.multiply(grad, scale)
		...
		variables_to_restore = self.net.get_variables_to_restore(tf.compat.v1.global_variables(), var_keep_dic)
		...
		print("Fixed.")
		sess.run(tf.compat.v1.assign(lr, self._learning_rate))
		last_compute_pic = 0
		iter = last_compute_pic + 1
		timer = Timer()
		while iter < 40000 + 1:
			if iter == 30000 + 1:
				sess.run(tf.compat.v1.assign(lr, self._learning_rate * 0.1))
			timer.tic()
			xml_name = self._xml_list[last_compute_pic]
			blobs = self.parse_xml(os.path.join(self._xml_path, xml_name))
			try:
				if iter % 1000 == 0:
					run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)
					run_metadata = tf.compat.v1.RunMetadata()
					rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, total_loss, summary, steps = self.net.train_step(sess, 
						blobs, train_op, merged, global_step, run_options, run_metadata)
					summary_writer.add_run_metadata(run_metadata, "step%d" % iter)
					summary_writer.add_summary(summary, steps)
				else:
					rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, total_loss, summary, steps = self.net.train_step(sess, 
						blobs, train_op, merged, global_step)
					summary_writer.add_summary(summary, steps)
			except:
